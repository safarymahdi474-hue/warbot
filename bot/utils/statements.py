...
def create_statement(user: User, text: str) -> CountryStatement | str:
    ...
    if not user.country_title:
        return "تو هنوز لقب/کشوری نداری."
...
def build_channel_text(statement: CountryStatement) -> str:
    return f"📜 <b>بیانیه رسمی {statement.country_title}</b>\n\n{statement.text}"
...
async def approve_statement(
    ...
    sent = await bot.send_message(
        ...
        text=build_channel_text(statement),
        parse_mode="HTML",
    )
